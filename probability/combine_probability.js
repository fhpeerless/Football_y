...
const PRESENT_JSON_PATH = path.join(PROJECT_ROOT, 'present.json');
const RESULT_DIR_PATH = path.join(PROJECT_ROOT, 'result');
...
async function getCurrentPeriod() {
    return new Promise((resolve, reject) => {
        try {
            ...
            if (!fs.existsSync(PRESENT_JSON_PATH)) {
                console.log('present.json文件不存在');
                resolve(null);
                return;
            }
            
            const fileContent = fs.readFileSync(PRESENT_JSON_PATH, 'utf8');
...
function findLatestPeriodFromFiles() {
    try {
        const resultDir = RESULT_DIR_PATH;
